...
import { Plus, Sliders, X, ChevronDown, ChevronRight } from 'lucide-react';
import { CHANNELS, CHANNEL_CATEGORIES, EFFECT_TYPES, EFFECT_TYPE_KEYS } from '../../lib/gameData/constants';
import { useGameStore } from '../../state/useGameStore';

/**
 * The channel rack — one row per mixer channel, each with its fader, mute and
 * its own insert-effect chain. Rows are grouped into collapsible instrument
 * families (드럼/건반/현악/관악…) so the rack stays legible as the roster grows.
 * Selecting a row drives which editor the centre pane shows; channels with a
 * plugin (currently just the drum machine) get a button that opens its window.
 */
export function ChannelRack() {
  ...
  // Categories start expanded; the player collapses families they aren't using.
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {CHANNEL_CATEGORIES.map((cat) => {
        const rows = CHANNELS.filter((c) => c.cat === cat.key);
        if (rows.length === 0) return null;
        const isCollapsed = collapsed.has(cat.key);
        // Surface a dot when a collapsed family still has the selected channel,
        // so it isn't "lost" while hidden.
        const hasSelected = rows.some((c) => c.key === selectedChannel);
        return (
          <div key={cat.key} className="flex flex-col gap-1.5">
            <button
              className="flex items-center gap-1.5 px-1 py-1 bg-transparent border-0 cursor-pointer text-left w-full"
              onClick={() => toggleCat(cat.key)}
              aria-expanded={!isCollapsed}
            >
              {isCollapsed ? <ChevronRight size={12} className="text-faint" /> : <ChevronDown size={12} className="text-faint" />}
              <span className="text-[10px] font-semibold uppercase tracking-wide text-muted">{cat.label}</span>
              <span className="text-[9px] font-mono text-faint">{rows.length}</span>
              {isCollapsed && hasSelected && <span className="w-1.5 h-1.5 rounded-full bg-accent2" title="선택된 악기가 여기 있어요" />}
            </button>
